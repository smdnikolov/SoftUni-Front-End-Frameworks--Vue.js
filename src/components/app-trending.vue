    ...
    <div v-else>
      ...
      <div v-else class="col-sm-4 center" style="text-align:center">
        <p style="font-size:35px;color=gray">
          There are no trending memes yet, vote on
          <router-link to="/fresh" style="font-size:35px">fresh&nbsp;</router-link>so they reach trending
        </p>
      </div>
    </div>
  </div>
      ...
      axios
        ...
        .then(data => {
          ...
          data.forEach(element => {
            if (element[1].upvotes > 10 && element[1].upvotes < 21) {
              this.memes.push({
                id: element[0],
